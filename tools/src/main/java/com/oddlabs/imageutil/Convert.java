...
System.out.println("outfile = " + outfile);
	...
	private static Layer loadFile(File file) throws IOException {
		...
		BufferedImage imageData = image.read(file);
...
//		assert image.getColorType() == PngImage.COLOR_TYPE_RGB || image.getColorType() == PngImage.COLOR_TYPE_RGB_ALPHA;
		int channels = image.getColorType() == PngConstants.COLOR_TYPE_RGB ? 3 : 4;
//		final byte[] bytes = getImageData(image);
		int[] ints = new int[width*height];
		try {
			boolean success = new PixelGrabber(imageData, 0, 0, width, height, ints, 0, width).grabPixels();
			assert success;
