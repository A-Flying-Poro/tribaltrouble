...
public final strictfp class DebugRender {
	...
	private final static float CIRCLE_DELTA = (float) StrictMath.PI/2;
	private final static float ANGLE_DELTA = (float) StrictMath.PI/20;
...
//		for (float z = origin_z; z < origin_z + height; z += SUBDIV)
	...
	private final static void drawCircle(float radius, float origin_x, float origin_y, float origin_z) {
		...
		for (float phi = 0f; phi < (float) StrictMath.PI*2; phi += ANGLE_DELTA) {
			x = radius*(float) StrictMath.cos(phi);
			y = radius*(float) StrictMath.sin(phi);
...
//		GL11.glDisable(GL11.GL_LIGHTING);
		GL11.glColor3f(r, g, b);
		for (float phi = 0; phi < (float) StrictMath.PI; phi += CIRCLE_DELTA) {
			GL11.glBegin(GL11.GL_LINE_LOOP);
			for (float rho = 0f; rho < (float) StrictMath.PI*2; rho += ANGLE_DELTA) {
				x = radius*(float) StrictMath.cos(rho);
				z = radius*(float) StrictMath.sin(rho);
				y = x*(float) StrictMath.sin(phi);
				x *= (float) StrictMath.cos(phi);
